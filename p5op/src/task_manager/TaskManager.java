...
public class TaskManager extends TextConsole {
	...
	Tasks tasks = Tasks.getTasksSingleton();
	Task current;
	...
	Function start = (args) ->{
		...
		if(tasks.getTask(args[0].toLowerCase())!= null){
			current= tasks.getTask(args[0].toLowerCase());
	...
	Function parent = (args) ->{
		...
		Task parent = tasks.getTask(args[0].toLowerCase());
	...
	Function status= (args) ->{
		...
		if(args.length==0){
			...
			Task t = tasks.getTask(args[0].toLowerCase());
			if(t!=null){
